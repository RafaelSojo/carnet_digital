...
exports.actualizarFotografia = async (req, res) => {
  ...
  if (!imagenBase64 || typeof imagenBase64 !== 'string') {
    return res.status(400).json({ mensaje: "Falta imagen en base64 o no es válida" });
  }
  ...
  try {
    ...
    if (!usuarioID) {
      return res.status(404).json({ mensaje: `No existe usuario ${usuarioID}` });
    }
    ...
    return res.status(500).json({ mensaje: "Error al actualizar", error: err.message });
  }
};
...
exports.obtenerFotografia = async (req, res) => {
  ...
  try {
    ...
    await registrarErrorEnBitacora(req.usuarioId, err.message);
    return res.status(500).json({ mensaje: "Error al obtener foto", error: err.message });
  }
};
...
exports.eliminarFotografia = async (req, res) => {
  ...
  try {
    ...
    if (!usuario || !usuario.fotografia) {
      return res.status(404).json({ mensaje: "No hay fotografía que eliminar" });
    }
    ...
    await registrarErrorEnBitacora(req.usuarioId, err.message);
    return res.status(500).json({ mensaje: "Error al eliminar foto", error: err.message });
  }
};
